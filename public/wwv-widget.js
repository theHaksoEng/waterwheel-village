...
(() => {
  ...
  function normalizeToken(t) {
    ...
    if (t.endsWith("es") && t.length > 2) {
      ...
      if (base.endsWith("s") || base.endsWith("x") || base.endsWith("z") || base.endsWith("ch") || base.endsWith("sh") || base.endsWith("o")) return base;
    }
  ...
  class WaterwheelChat extends HTMLElement {
    constructor() {
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .pill { display:inline-flex; align-items:center; gap:6px; border:1px solid #e2e8f0; border-radius:9999px; padding:6px 10px; font-size:13px; background:#f8fafc; color:#0f172a }
    ...
    renderWordlist() {
      ...
      this.wordlist.forEach(({ en, fi }) => {
        ...
        const pill = ce("div", { className: "pill" });
        if (this.learned.has(key)) pill.classList.add("learned");

        // main word (click inserts into input)
        const main = ce("span", { textContent: showFi && fi ? en + " · " + fi : en });
        ...
        // “Say” chip — plays audio + shows a tip
        const say = ce("button", { className: "say", type: "button", textContent: "Say", title: "Hear pronunciation + tip" });
        say.addEventListener("click", (ev) => {
          ev.stopPropagation();
          this.pronounceWord(en);
        });
        pill.appendChild(say);
    ...
    // Build a simple “slow” version (letters spaced; phrases word-by-word)
    slowPronounce(text) {
      const t = String(text || "").trim();
      if (!t) return "";
      return t.split(/\s+/).map(w => w.split("").join(" ").replace(/\s+/g, " ")).join("   ");
    }
    ...
    async startLesson() {
      ...
      // Load wordlist first (object-with-words or array)
  ...
  // Auto-mount helper for sites that prefer a simple container
