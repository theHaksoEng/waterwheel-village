...
(() => {
    ...
    class WaterwheelChat extends HTMLElement {
      ...
      renderWordlist() {
        ...
        this.wordlist.forEach(({ en, fi }) => {
          ...
          const pill = ce("button", { className: "pill", type: "button" });
          if (this.learned.has(key)) pill.classList.add("learned");
          pill.textContent = showFi && fi ? en + " · " + fi : en;
    ...
    // If a page includes: <div id="wwv-root"></div> and the script, inject the element.
