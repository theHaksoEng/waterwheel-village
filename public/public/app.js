...
(function () {
  function init() {
    ...
    // Config
    // =====================================
    // "" = same-origin (use your Express server domain automatically)
    // If you want to hardcode Render, set e.g.:
    ...
    // Single, clean playVoice (no duplicates)
    async function playVoice(text, voiceId) {
      // Prefer free browser TTS or when no voiceId is provided
      if (useBrowserTTSChk?.checked || !voiceId) {
        await speakBrowser(text);
        return;
      }
      // Else try backend ElevenLabs; on error fallback to browser
      try {
        ...
        await new Promise((resolve) => {
          const done = () => { voiceOutput.onended = null; voiceOutput.onerror = null; resolve(); };
          voiceOutput.onended = done;
          voiceOutput.onerror = done;
          voiceOutput.play().catch(() => done());
        });
      } catch (err) {
    ...
    // =====================================
    // Start chat (fixed Enter handling)
    // =====================================
    ...
    // Allow changing the name any time
    if (changeNameBtn) {
    ...
    async function startLesson(month, chapter) {
      ...
      try {
        ...
        if (lesson.welcomeText) {
          ...
          await playVoice(lesson.welcomeText, "fEVT2ExfHe1MyjuiIiU9"); // McArthur
        }
    ...
    window.startLesson = startLesson; // used by the chapter buttons
    ...
    // =====================================
    // Mic + meter + SpeechRecognition + fallback recorder
    // =====================================
    ...
    // WebSpeech + fallback recorder -> /stt
  ...
  // Make sure we only run after the DOM exists
