...
if (useRedis) {
  ...
  redisClient.on("error", (err) => logger.error({ err: err.stack, code: "REDIS_ERROR" }, "Redis connection error"));
  (async () => {
    try {
...
app.post("/chat", async (req, res) => {
  try {
    ...
    const replyText = await callChatbase(sessionId, [...(systemPrompt ? [systemPrompt] : []), ...previous, { role: "user", content: sanitizedText }], process.env.CHATBASE_BOT_ID, process.env.CHATBASE_API_KEY);
...
app.post("/speakbase", async (req, res) => {
  try {
    ...
    if (detectedCharacter) {
      ...
      logger.debug(`Session ${sessionId}: Using existing character ${detectedCharacter}`);
    } else {
    ...
    if (detectedCharacter === "mcarthur" && previous.length === 0 && !sessionData.studentName) {
      ...
      if (detectedCharacter === "mcarthur" && !sessionData.studentName && sanitizedUserMessage) {
        ...
        if (name) {
          ...
          storeChatHistory(sessionId, [...previous, { role: "user", content: sanitizedUserMessage || sanitizedText }, { role: "assistant", content: replyText }]);
    ...
    const voiceResponse = await axios({
      ...
      logger.error({ error: error.stack, code: "ELEVENLABS_ERROR" },
      "ElevenLabs API request failed");
      throw streamlinedError(error, "ELEVENLABS_ERROR");
    });
